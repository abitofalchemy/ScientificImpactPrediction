...
def procjson_cleaned_docs_from(in_json_fname=""):
...
#   tweets['hlinks'] = tweets['text'].apply(lambda t: [t.strip(l) for l in re.findall(r'(https?://\S+)', t) if 'http' in t])
  ...
  if os.path.exists('Results/tweets_cleaned.tsv'): print 'Saved file: Results/tweets_cleaned.tsv'
...
def main():
  ...
  procjson_cleaned_docs_from(args['jsonfile'])
